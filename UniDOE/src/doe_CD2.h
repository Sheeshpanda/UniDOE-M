...
#ifndef CPP_VERSION_CD2_H
#define CPP_VERSION_CD2_H

class CD2: public Criteria {

    vector<double> D1, tempD1;
    vector<vector<double> > xc, D2, tempD2;
public:
    CD2(vector<vector<double> > init,int nsamp_init,int nv_init, int nlevel_init): Criteria(nsamp_init,nv_init, nlevel_init) {
        init_design(init);
        evaluate_criteria();
    }
